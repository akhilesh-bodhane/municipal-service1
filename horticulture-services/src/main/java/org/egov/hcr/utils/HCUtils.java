...
public class HCUtils {
	...
	public RequestInfoWrapper prepareRequestForLocalization(StringBuilder uri, RequestInfo requestInfo, String locale,
		...
		uri.append(hcConfiguration.getLocalizationHost()).append(hcConfiguration.getLocalizationSearchEndpoint())
	...
	public void validateJsonAddUpdateData(ServiceRequest serviceRequest, String applicationType) {
		String responseText = "";
		try {
			Gson gson = new Gson();
	...
	private String commonValidation(JSONObject jsonValidator, JSONObject jsonRequested) {
		...
		try {
			for (String key : keyValidateList) {
				...
				if (key.equals("is_Active") || key.equals("media") || key.equals("wfDocuments")
						|| key.equals("isRoleSpecific"))
					continue;
				else {
						...
						if (jsonRequested.get(key) instanceof JSONArray) {
							...
							for (Object json : jsonarray) {
								if (jsonRequested.get(key) instanceof JSONObject) {
									...
									HashMap<String, Object> yourHashMap = new Gson().fromJson(mJSONObject.toString(),
											HashMap.class);

									for (Map.Entry<String, Object> entry : yourHashMap.entrySet()) {
										if (entry.getValue().toString().contains("</script>")
												|| entry.getValue().toString().contains("&lt;/script&gt;")) {
											responseText.append(entry.getKey().toString() + ":[Invalid data]");
											responseText.append(",");
										} else
											responseText = xxsFilterPatern(entry.getValue().toString(),
													entry.getKey().toString(), responseText);
									}
								} else {
									if (json.toString().contains("</script>")
											|| json.toString().contains("&lt;/script&gt;")) {
	...
	private StringBuilder xxsFilterPatern(String dataReq, String key, StringBuilder responseText) {
		...
		for (String validateStrPattern : validatePatternList) {
			if (!dataReq.equals("") && !dataReq.equals(null)) {
				validatePattern = Pattern.compile(validateStrPattern, Pattern.CASE_INSENSITIVE);
