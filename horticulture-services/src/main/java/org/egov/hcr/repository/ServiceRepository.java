package org.egov.hcr.repository;
...
public class ServiceRepository {
		...
		try {
			if (!requestData.getService_request_id().isEmpty()) {
				...
				if (requestData.getRequestInfo().getUserInfo().getType().equals("CITIZEN")) {
					actualResult = (JSONArray) jdbcTemplate
							.query(queryBuilder.SELECT_SERVICE_DETAIL_FOR_CITIZEN,
									new Object[] { requestData.getService_request_id(),
											requestData.getAuditDetails().getCreatedBy(), requestData.getServiceTypeName() },
									rowMapper);
		...
		try {
			...
			allRoles = rest.postForObject(
					hcConfiguration.getEgovHRMShost().concat(hcConfiguration.getEgovHRMSSearchEndpoint()).concat("?")
							.concat("roles=" + role + "&tenantId=" + request.getServices().get(0).getCity()),
	...
	public JSONArray getServiceRequestList() {
		try {
			...
			JSONArray actualResult = (JSONArray) jdbcTemplate.query(queryBuilder.GET_CREATED_TIME, new Object[] {},
					rowMapper);
