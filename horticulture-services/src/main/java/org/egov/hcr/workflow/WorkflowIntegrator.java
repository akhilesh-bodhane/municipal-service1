...
public class WorkflowIntegrator {
	...
	private static final String MCMODULENAMEVALUE = "MCHORTICULTURE";

	private static final String UTMODULENAMEVALUE = "UTHORTICULTURE";

	private static final String FRMODULENAMEVALUE = "FRHORTICULTURE";
	...
	public boolean callWorkFlow(ServiceRequest request, String service_request_id, String role, String employeeUuid)
		...
		if (!request.getServices().isEmpty()) {
			...
			for (ServiceRequestData servicerequestdata : request.getServices()) {
				...
				if (servicerequestdata.getServiceTypeName().equals("UT"))
					obj.put(MODULENAMEKEY, UTMODULENAMEVALUE);
				else if (servicerequestdata.getServiceTypeName().equals("MC"))
					obj.put(MODULENAMEKEY, MCMODULENAMEVALUE);
				else if (servicerequestdata.getServiceTypeName().equals("FR"))
					obj.put(MODULENAMEKEY, FRMODULENAMEVALUE);
...
//						states.forEach(ss -> {
//							if (newState.equals(ss.getUuid())) {
//								ss.getActions().forEach(aa -> {
//									aa.getRoles().forEach(r -> {
//										if (null != roles && !roles.isEmpty()) {
//											roles.set(0, roles.get(0).concat(",").concat(r));
//										} else {
//											roles.add(r);
//										}
//										found.set(true);
