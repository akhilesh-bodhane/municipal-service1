...
public class TLQueryBuilderNIUA {
	...
	public static final String QUERY_TL_PUBLIC_DASHBOARD = "select totalApplicationsReceived, totalApplicationsApproved, case when timeTakenForApproval <=0 then 1 else timeTakenForApproval end as timeTakenForApproval\r\n"
			...
			+ "	count(case when ett.action = 'APPROVE' then 1 end) totalApplicationsApproved,\r\n"
			+ "	ceil(SUM(case when ett.action = 'APPROVE' then to_timestamp(ett.lastmodifiedtime / 1000)::date - to_timestamp(ett.createdtime / 1000)::date end)/ count(case when ett.action = 'APPROVE' then 1 end)) timeTakenForApproval\r\n"
