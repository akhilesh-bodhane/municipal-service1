...
public class ServiceRequestRepository {
	...
	public static final String SERVICE_DASHBOARD_SEARCH_WITH_DETAILS = "select max(pg.serviceRequestId) serviceRequestId, max(pg.servicecode) servicecode, max(pg.category) category, max(pg.createdtime) createdtime, max(pg.createdby) createdby, max(pg.lastmodifiedtime) lastmodifiedtime, max(pg.lastmodifiedby) lastmodifiedby, max(ad.mohalla) mohalla, max(pg.status) status, max(pg.slaendtime) slaendtime, max(pg.description) description, max(pg.rating) rating, max(us.\"name\") as name,	max(us.mobilenumber) mobilenumber, json_agg(json_build_object('uuid', ac.uuid, 'by', ac.by, 'status', ac.status,'when', ac.\"when\", 'tenantId', ac.tenantId, 'businessKey', ac.businessKey, 'action', ac.\"action\", 'media', ac.media)) actionHistory from eg_pgr_service pg left join eg_pgr_address ad on pg.addressid = ad.uuid left join eg_user us on pg.createdby = cast(us.id as varchar) left join eg_pgr_action ac on pg.serviceRequestId = ac.businesskey where ac.businesskey is not null and ac.status is not null and ";
