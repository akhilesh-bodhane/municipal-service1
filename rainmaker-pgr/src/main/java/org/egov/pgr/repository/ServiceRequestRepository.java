...
public class ServiceRequestRepository {
	...
	public static final String GRIEVANCE_SEARCH = "select row_number() over(order by servicecode,category,status,\"source\") sequencenum, servicecode,category,status,\"source\",\r\n"
			...
			+ "SUM(case when status = 'closed' then to_char(justify_hours(interval '1 sec' * (to_timestamp(slaendtime)::date - to_timestamp(createdtime)::date)), 'DD')::int else 0 end) completionDays from eg_pgr_service where ";
