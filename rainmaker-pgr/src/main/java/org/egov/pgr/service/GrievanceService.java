...
public class GrievanceService {
	...
	private void enrichserviceRequestForcreate(ServiceRequest serviceRequest) {
		...
		for (int servReqCount = 0; servReqCount < serviceReqs.size(); servReqCount++) {
			...
			try {
				...
				if (null != actionInfo) {
					...
					ActionInfo newActionInfo = ActionInfo.builder().uuid(UUID.randomUUID().toString())
							.businessKey(currentId).action(WorkFlowConfigs.ACTION_OPEN).assignee(null).by(by)
			...
			try {
				...
				if (!StringUtils.isEmpty(employeeCode)) {
					...
					if (!CollectionUtils.isEmpty(userResponse.getUser())) {
						ActionInfo newActionInfo = ActionInfo.builder().uuid(UUID.randomUUID().toString())
								.businessKey(currentId).action(WorkFlowConfigs.ACTION_ASSIGN)
								.assignee(String.valueOf(userResponse.getUser().get(0).getId())).by(by)
								.when(auditDetails.getCreatedTime() + 1).tenantId(tenantId)
			...
			if ((Integer) serivceDefs.get(3) > 0) {
				...
				// servReq.setSlaEndTime(auditDetails.getCreatedTime() +
				// Long.parseLong(serivceDefs.get(3).toString())*24*60*60*1000);
	...
	/**
	 * Since the request and response formats of PGR are different, indexer
	 * operations of INDEX and LEGACYINDEX will be affected. This method ensures the
	 * following: 1. Data format being sent to indexer is of the same type for
	 * create, update as that of search as LEGACYINDEX makes use of search 2. Data
	 * during update, takes the entire actionhistory alongwith the currently
	 * performed action.
	 * 
	 * @param request
	 * @return
	...
	private ServiceResponse dataTranformationForIndexer(ServiceRequest request, boolean isCreate) {
		/**
		 * This might seem inefficient but in our use-case, create and update happen for
		 * just one compliant 95% of the times, so loop runs just once.
		 */
		...
		if (isCreate) {
			...
			for (int i = 0; i < request.getServices().size(); i++) {
				...
				ServiceReqSearchCriteria serviceReqSearchCriteria = ServiceReqSearchCriteria.builder()
						.tenantId(request.getServices().get(i).getTenantId()).serviceRequestId(serviceRequestIds)
						.build();
				ServiceResponse serviceResponse = mapper.convertValue(
						getServiceRequestDetails(request.getRequestInfo(), serviceReqSearchCriteria),
				...
				AuditDetails auditDetails = AuditDetails.builder()
						.createdBy(serviceResponse.getServices().get(0).getAuditDetails().getCreatedBy())
	...
	private void enrichServiceRequestForUpdate(ServiceRequest request) {
		...
		for (int index = 0; index < serviceReqs.size(); index++) {
			...
			// If the action is reopen, then go through the action history & find whether it
			// is reopened
			// 1st time(last resolved by LME) or 2nd time(last resolved by escalation
			// officer1)
			...
			String role = pGRUtils.getPrecedentRole(
			...
			actionInfo.setBy(auditDetails.getLastModifiedBy() + ":" + role);
			actionInfo.setWhen(auditDetails.getLastModifiedTime());
			actionInfo.setTenantId(service.getTenantId());

			// If GRO resolves/reject any complaint (LME assigned complaint/ Escalated
			// complaint) then we set the assignee.
			...
			// If escalated complaint is resolved or reject by escalation officer then set
			// the assignee value of escalation officer
	...
	public Object getServiceRequestDetails(RequestInfo requestInfo, ServiceReqSearchCriteria serviceReqSearchCriteria) {
		...
		if ((codes.contains(PGRConstants.ROLE_ESCALATION_OFFICER1))
			 ...
			 * STATUS_ESCALATED_LEVEL1_PENDING) ||
			 * serviceReqSearchCriteria.getStatus().contains(WorkFlowConfigs.
			 * STATUS_ESCALATED_LEVEL2_PENDING))){
			 */
			...
			try {
				 ...
				 * status.add(WorkFlowConfigs.STATUS_ESCALATED_LEVEL1_PENDING);
				 * status.add(WorkFlowConfigs.STATUS_ESCALATED_LEVEL2_PENDING);
				 * System.out.println("Status of EO1 & EO2 : " + status.toString()); } else if
				 * (codes.contains(PGRConstants.ROLE_ESCALATION_OFFICER1) &&
				 * codes.contains(PGRConstants.ROLE_PGR_LME)) {
				 * status.remove(WorkFlowConfigs.STATUS_ESCALATED_LEVEL2_PENDING);
				 * status.add(WorkFlowConfigs.STATUS_ESCALATED_LEVEL1_PENDING);
				 * status.add(WorkFlowConfigs.STATUS_ASSIGNED);
				 * System.out.println("Status of EO1 and LME : " + status.toString()); } else if
				 * (codes.contains(PGRConstants.ROLE_ESCALATION_OFFICER2) &&
				 * codes.contains(PGRConstants.ROLE_PGR_LME)) {
				 * status.remove(WorkFlowConfigs.STATUS_ESCALATED_LEVEL1_PENDING);
				 * status.add(WorkFlowConfigs.STATUS_ESCALATED_LEVEL2_PENDING);
				 * status.add(WorkFlowConfigs.STATUS_ASSIGNED);
				 * System.out.println("Status of EO2 and LME : " + status.toString()); } else if
				 * (codes.contains(PGRConstants.ROLE_ESCALATION_OFFICER1)) {
				 * status.add(WorkFlowConfigs.STATUS_ESCALATED_LEVEL1_PENDING);
				 * status.remove(WorkFlowConfigs.STATUS_ESCALATED_LEVEL2_PENDING);
				 * System.out.println("Status of EO1 : " + status.toString()); } else if
				 * (codes.contains(PGRConstants.ROLE_ESCALATION_OFFICER2)) {
				 * status.add(WorkFlowConfigs.STATUS_ESCALATED_LEVEL2_PENDING);
				 * status.remove(WorkFlowConfigs.STATUS_ESCALATED_LEVEL1_PENDING);
	 ...
	 * @author vishal
	 */
	public Object getServiceRequestDetailsForPlainSearch(RequestInfo requestInfo,
	...
	/**
	 * This method fetches all the codes of departments that the employee belongs
	 * to.
	 * 
	 * @param serviceReqSearchCriteria
	 * @param requestInfo
	...
	public Object getCount(RequestInfo requestInfo, ServiceReqSearchCriteria serviceReqSearchCriteria) {
		...
		/*
		 * if (null == response) return pGRUtils.getDefaultCountResponse(requestInfo);
		 */
	...
	/**
	 * This method populates timeline information of the service request with user
	 * details of the actors on the complaint This method populates the locality
	 * field of the addressDetail object with name (english) of the mohalla.
	 * 
	 * @param requestInfo
	 * @param response
	 * @return
	...
	public ServiceResponse enrichResult(RequestInfo requestInfo, ServiceResponse response) {
		...
		if (!CollectionUtils.isEmpty(addresses)) {
			...
			/**
			 * When CSR searches, complaints in the result belong to multiple tenants.
			 * Inorder to populate mohalla value, we need tenant of every complaint.
			 */
			...
			for (String tenantId : tenantIds) {
				if (!StringUtils.isEmpty(tenantId)) {
					...
					if (!CollectionUtils.isEmpty(mapOfTenantIdAndMohallaCodes.get(tenantId))) {
						tenantWiseMap = getMohallNames(requestInfo, tenantId,
								mapOfTenantIdAndMohallaCodes.get(tenantId),
		...
		if (!CollectionUtils.isEmpty(mapOfMohallaCodesAndNames.keySet())) {
			for (Service service : response.getServices()) {
				if (null != service) {
					if (null != service.getAddressDetail()) {
						if (!StringUtils
								.isEmpty(mapOfMohallaCodesAndNames.get(service.getAddressDetail().getMohalla()))) {
							service.getAddressDetail().setLocality(
									mapOfMohallaCodesAndNames.get(service.getAddressDetail().getMohalla()));
						}
					}
				}
			}
		 ...
		 */
		String tenantId = response.getServices().get(0).getTenantId().split("[.]")[0]; // citizen is state-level no
																						// point in sending ulb level
																						// tenant.
	 ...
	 * @return
	 */
	public Map<String, String> getMohallNames(RequestInfo requestInfo, String tenantId, List<String> mohallaCodes,
			String hierarchyType, String boundaryType) {
		StringBuilder uri = new StringBuilder();
		RequestInfoWrapper request = pGRUtils.prepareRequestForLocation(uri, requestInfo, boundaryType, tenantId,
				hierarchyType, mohallaCodes);
	 ...
	 * @author Tonmoy
	 */
	public Object fetchAutoroutingEscalationMap(RequestInfo requestInfo, String tenantId, String category,
			String sector) {
		StringBuilder uri = new StringBuilder();
		MdmsCriteriaReq mdmsCriteriaReq = pGRUtils.prepareAutoroutingEscalationMapSearchMdmsRequestByCategoryAndSector(
				uri, tenantId, category, sector, requestInfo);
		Object response = null;
		try {
			response = serviceRequestRepository.fetchResult(uri, mdmsCriteriaReq);
		} catch (Exception e) {
	 ...
	 * @author Tonmoy
	 */
	public Map<String, List<String>> fetchEscalatingOfficers(RequestInfo requestInfo, String tenantId,
			Service servReq) {
	...
	/**
	 * Fetches the category, department, Service type, sla hours for the respective
	 * service type
	 * 
	 * @param serviceReq
	 * @param requestInfo
	...
	public List<Object> getServiceType(Service serviceReq, RequestInfo requestInfo) {
		...
		try {
			...
			String locale = null;
			try {
				locale = requestInfo.getMsgId().split("[|]")[1]; // Conventionally locale is sent in the first index of
																	// msgid split by |
			...
			log.info("Get data from servicedefs- categories:" + categories + " ,departments: " + departments
					+ " ,serviceTypes:" + serviceTypes + " ,slaHours:" + slaHours);

			if (null == localizedMessageMap.get(locale + "|" + tenantId)) // static map that saves code-message pair
																			// against locale | tenantId.
				getLocalisedMessages(requestInfo, tenantId, locale, PGRConstants.LOCALIZATION_MODULE_NAME);

			if (!CollectionUtils.isEmpty(serviceTypes))
				serviceType = localizedMessageMap.get(locale + "|" + tenantId)
						.get(PGRConstants.LOCALIZATION_COMP_CATEGORY_PREFIX + serviceTypes.get(0).toUpperCase()); // result
																													// set
																													// is
																													// always
																													// of
	...
	public Map<String, List<String>> fetchCategoriesForEscalationOfficer(RequestInfo requestInfo, String tenantId) {
		...
		List<String> sectorList2 = new ArrayList<>();
		String employeeCode = null;
		try {
			...
			if (null != result) {				
				...
				/*
				 * for (int i = 0; i < objList.size(); i++) { List<String>
				 * escalationOfficer1List = JsonPath.read(objList.get(i),
				 * PGRConstants.AUTOROUTING_ESCALATING_OFFICER1_JSONPATH); if
				 * (!CollectionUtils.isEmpty(escalationOfficer1List)) { if
				 * (escalationOfficer1List.contains(requestInfo.getUserInfo().getUserName())) {
				 * if (CollectionUtils.isEmpty(categoryList1)) categoryList1 = new
				 * ArrayList<String>(); categoryList1 .add(JsonPath.read(objList.get(i),
				 * PGRConstants.AUTOROUTING_CATEGORY_JSONPATH)); } } }
				 */
		...
		categoryMap.put(PGRConstants.MDMS_SECTOR_MASTER_NAME_SMALL, sectorList2);
	...
	public Object fetchCategoriesFromAutoroutingEscalationMap(RequestInfo requestInfo, String tenantId) {
		StringBuilder uri = new StringBuilder();
		MdmsCriteriaReq mdmsCriteriaReq = pGRUtils.prepareCategoryMdmsRequestByEscalationOfficer(uri, tenantId,
				requestInfo);
		Object response = null;
		try {
			response = serviceRequestRepository.fetchResult(uri, mdmsCriteriaReq);
	...
	public Object fetchPgrDepartment(RequestInfo requestInfo, String tenantId, String departmentCode) {
		StringBuilder uri = new StringBuilder();
		MdmsCriteriaReq mdmsCriteriaReq = pGRUtils.prepareSearchRequestForPgrDepartment(uri, tenantId, departmentCode,
				requestInfo);
		Object response = null;
		try {
			response = serviceRequestRepository.fetchResult(uri, mdmsCriteriaReq);
	 ...
	 * @author Tonmoy
	 */
	public Object getEscalationServiceRequestDetails(RequestInfo requestInfo,
			ServiceReqSearchCriteria serviceReqSearchCriteria) {
		StringBuilder uri = new StringBuilder();
		SearcherRequest searcherRequest = null;
		try {
	 ...
	 * @param serviceReqSearchCriteria
	 */
	public void enrichEscalationSearchRequest(RequestInfo requestInfo,
			ServiceReqSearchCriteria serviceReqSearchCriteria) {
	...
	private Double getComplaintCountForEscalationOfficer(RequestInfo requestInfo,
		...
		// If the user is escalation officer then 1st get the pending complaint count of
		// 1st level & get the pending complaint count of 2nd level
		// & merge these two results in the response. Because same user can be the
		// escalation officer at 1st
		// level in one department & 2nd level officer in other department
	...
	public Object getServiceRequestDetailsDB(RequestInfo requestInfo,
			ServiceReqSearchCriteria serviceReqSearchCriteria) {
		StringBuilder uri = new StringBuilder();
		SearcherRequest searcherRequest = null;
		try {
		...
		Object response = null;
		try {
			response = serviceRequestRepository.fetchData(serviceReqSearchCriteria);
		} catch (Exception e) {
			if (e.getMessage().equals(ErrorConstants.NO_DATA_MSG))
				return pGRUtils.getDefaultServiceResponse(requestInfo);
			else
