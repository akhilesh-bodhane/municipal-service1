...
public class PGRRequestValidator {
	...
	public void validateSearch(ServiceReqSearchCriteria criteria, RequestInfo requestInfo) {
		 ...
		 * cal.getTime().getTime()) || (criteria.getEndDate() != null &&
		 * criteria.getEndDate() > cal.getTime().getTime())) {
	...
	public void validateActionsOnCurrentStatus(ServiceRequest serviceRequest, Map<String, String> errorMap) {
		...
		for (int index = 0; index < serviceRequest.getServices().size(); index++) {
			...
			currentStatus = service.getStatus().toString();
			...
			log.info("Service Request Id 1 : " + service.getServiceRequestId() + ", Service Request Status : "
					+ currentStatus);
			if (currentStatus == null || (currentStatus != null && currentStatus.isEmpty()))
				currentStatus = pgrUtils.getCurrentStatus(history);
			log.info("Service Request Id 2 : " + service.getServiceRequestId() + ", Service Request Status : "
					+ currentStatus);
