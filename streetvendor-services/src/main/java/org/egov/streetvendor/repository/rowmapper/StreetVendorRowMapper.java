...
@Component
public class StreetVendorRowMapper  implements ResultSetExtractor<List<StreetVendorData>> {
	...
	public List<StreetVendorData> extractData(ResultSet rs) throws SQLException, DataAccessException {
		Map<String, StreetVendorData> sepMap = new HashMap<>();
		
		List<StreetVendorData> StreetVendorDataList = new ArrayList<>();

		try {
			while (rs.next()) {
				String id = rs.getString("vendor_uuid");
				StreetVendorData streetvendorDetails = new StreetVendorData();
				
				streetvendorDetails = streetvendorDetails.builder().build();
				if (!sepMap.containsKey(id)) {
					...
					sepMap.put(id, streetvendorDetails);
