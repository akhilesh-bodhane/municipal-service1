...
public class STALLQueryBuilder {
	...
	public static final String GET_STALL_APPLICATION_QUERY = "SELECT TS.id, TS.applicationn_id, TS.name, TS.mobile_no, TS.festival, TS.from_date, TS.to_date, TS.sector, TS.village , TS.paymentstatus , TS.stall_size, TS.address_details, TS.landmark, TS.tenant_id, TS.total_amount, TS.is_active, TS.created_by, TS.created_time, TS.last_modified_by, TS.last_modified_time, TS.no_o_days, TS.fees_per_day, TS.application_status, TS.main_amount, TS.gst_amont, TS.nominee_name, TS.relation, TS.latitude, TS.longitude, \n"
			...
			+ "	FROM temporary_stall_application_detail TS inner join temporary_stall_application_document SD on TS.id=SD.id and TS.tenant_id=SD.tenant_id \n"
	...
	public static final String SCHEDULAR = "SELECT TS.id, TS.applicationn_id,  TS.paymentstatus , TS.application_status \n"
			+ "	FROM temporary_stall_application_detail TS \n"
	...
	public static final String GET_STALL_DEMAND_DETAIL_QUERY ="select id , taxheadcode from egbs_demanddetail_v1 edv where demandid  in (select id  from egbs_demand_v1 edv where consumercode =(case when ?  <>'' then ?  else consumercode end) )";
