...
public class SWCalculationServiceImpl implements SWCalculationService {
	...
	public List<Calculation> getCalculation(CalculationReq request) {
		...
		if (request.getIsconnectionCalculation()) {
			...
			if (finYear.equalsIgnoreCase("2023-24")) {
				...
				if(request.getCalculationCriteria().get(0).getSewerageConnection().getSwProperty().getAuditDetails().getCreatedTime().toString().compareTo("1686767400000") < 0 ) {
	...
	public List<Calculation> getEstimation(CalculationReq request) {
		...
		if (finYear.equalsIgnoreCase("2023-24")) {
			System.out.println("Inside Fin Yr 2023-24");
			//if(Date.compareTo("13-06-2023") < 0) {
			if(request.getCalculationCriteria().get(0).getSewerageConnection().getAuditDetails().getCreatedTime().toString().compareTo("1686767400000") < 0 ) {
