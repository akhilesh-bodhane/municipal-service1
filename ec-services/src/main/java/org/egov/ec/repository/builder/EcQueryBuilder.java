...
public class EcQueryBuilder {
	...
	public static final String GET_VENDOR_DETAIL_SEARCH = "select distinct on (cov_no) *,(select case when count(*)<5 then (count(*)+1) else 5 end from egec_violation_master v where v.license_no_cov=egec_vendor_registration_master.cov_no  ) as numberOfViolation from public.egec_vendor_registration_master \n"
...
//			+ " and violation.tenant_id=? and UPPER(challan.challan_id) like concat('%',case when UPPER(?)<>'' then UPPER(?) else UPPER(challan.challan_id) end,'%') order by violation.last_modified_time desc";
	...
	public static final String SEARCH_VIOLATION_MASTER = "select 	max(case when store.item_store_deposit_date < now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION' when challan.challan_status = 'CLOSED' and store.violation_uuid is not null then 'RELEASED FROM STORE' when challan.challan_status = 'CLOSED' and store.violation_uuid is null then 'RELEASED ON GROUND' else challan.challan_status end) as challan_status, 	max(coalesce(ch.head_amount, 0)) as fineAmount, max(coalesce(chs.head_amount, 0)) as penaltyAmount,	max(violation.violation_uuid) as violation_uuid,max(contact_number) contact_number,	max(challan.challan_uuid) as challan_uuid,max(challan.tenant_Id) as tenant_Id,max(challan.challan_Id) as challan_Id,max(violation.encroachment_type) as encroachment_type,max(violation.violation_date) as violation_date,max(violation.violator_name) as violator_name,max(violation.sector) as sector,	max(violation.si_name) si_name,case when max(item.violation_item_uuid) is not null then json_agg(json_build_object('itemName', item.item_name, 'quantity', item.quantity)) else json_agg(json_build_object()) end violation_item, max(payment.payment_status) payment_status from public.egec_violation_master violation left join public.egec_challan_master challan on violation.violation_uuid = challan.violation_uuid left join public.egec_payment payment on 	violation.violation_uuid = payment.violation_uuid left join egec_store_item_register store on store.violation_uuid = violation.violation_uuid left join egec_challan_detail ch on ch.budget_head = 'FINE_AMOUNT' and ch.challan_uuid = challan.challan_uuid left join  egec_challan_detail chs on chs.budget_head = 'PENALTY_AMOUNT' and chs.challan_uuid = challan.challan_uuid left join public.egec_violation_detail item on violation.violation_uuid = item.violation_uuid where case when store.item_store_deposit_date < now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION' when challan.challan_status = 'CLOSED' and store.violation_uuid is not null then 'RELEASED FROM STORE' when challan.challan_status = 'CLOSED'	and  store.violation_uuid is null then 'RELEASED ON GROUND' else challan.challan_status end = case when ?<>'' then ? else case when store.item_store_deposit_date < now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION' 		when challan.challan_status = 'CLOSED' and store.violation_uuid is not null then 'RELEASED FROM STORE' when challan.challan_status = 'CLOSED' and store.violation_uuid is null then 'RELEASED ON GROUND' else challan.challan_status end end and violation.violation_date >= case when ?<> '' then DATE(?) 		else violation.violation_date end and violation.violation_date <= case when ?<> '' then DATE(?) else violation.violation_date end and violation.encroachment_type = case when ?<>'' then ? else violation.encroachment_type end and violation.si_name = case when ?<>'' then ? else violation.si_name end and violation.sector = case when ?<>'' then ? else violation.sector end and challan.challan_Id like concat(concat('%', case when ?<> '' then ? else challan.challan_Id end),'%') group by challan.challan_Id";
	...
	public static final String SEARCH_VIOLATION_MASTER_COUNT = "select violation.Violation_uuid,violation.encroachment_type, violation.violation_date, violation.sector, violation.si_name, violation.created_time,\r\n"
			...
			+ "JOIN public.egec_challan_master challan on violation.violation_uuid=challan.violation_uuid  \r\n"
			+ "JOIN public.egec_payment payment on violation.violation_uuid = payment.violation_uuid\r\n"
			+ "LEFT JOIN public.egec_store_item_register store on violation.violation_uuid = store.violation_uuid\r\n"
			+ "where violation.violation_date >= CASE WHEN ?<>'' THEN DATE(?) ELSE violation.violation_date END   \r\n"
