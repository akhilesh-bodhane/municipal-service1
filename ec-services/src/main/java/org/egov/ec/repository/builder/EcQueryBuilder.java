...
public class EcQueryBuilder {
	...
	public static final String GET_SPIC_VENDOR_DETAIL_SEARCH = "select distinct on (covno) *,(select case when count(*)<5 then (count(*)+1) else 5 end from egec_violation_master v where v.license_no_cov=egec_spic_vendor_details.covno  ) as numberOfViolation from egec_spic_vendor_details \n"
			+ "where covno like ? or vendorname ilike ?";
