...
//			+ " and violation.tenant_id=? and UPPER(challan.challan_id) like concat('%',case when UPPER(?)<>'' then UPPER(?) else UPPER(challan.challan_id) end,'%') order by violation.last_modified_time desc";
	...
	public static final String GET_DUPLICATE_ECHLLAN_DATA ="SELECT   \r\n"
			...
			+ "    ecm.challan_status,\r\n"
			+ "    ecm.challan_amount,\r\n"
			+ "    TO_CHAR(TO_TIMESTAMP(ecm.created_time / 1000), 'DD-MM-YYYY HH24:MI:SS') AS challan_date,\r\n"
			+ "    evm.encroachment_type,\r\n"
			+ "    evm.contact_number, \r\n"
			+ "    evm.violator_name,\r\n"
			+ "    evm.number_of_violation,             \r\n"
			+ "    ep.payment_mode,                   \r\n"
			+ "    ep.payment_status,\r\n"
			+ "    STRING_AGG(DISTINCT evd.item_name, ', ') AS item_names  \r\n"
			+ "FROM egec_challan_detail ecd\r\n"
			+ "INNER JOIN egec_challan_master ecm ON ecm.challan_uuid = ecd.challan_uuid  \r\n"
			+ "INNER JOIN egec_violation_master evm ON evm.violation_uuid = ecm.violation_uuid  \r\n"
			+ "INNER JOIN egec_violation_detail evd ON evd.violation_uuid = evm.violation_uuid\r\n"
			+ "INNER JOIN egec_payment ep ON ep.challan_uuid = ecm.challan_uuid           \r\n"
			+ "WHERE ecd.tenant_id = 'ch.chandigarh'\r\n"
			+ "  AND ep.payment_status = 'PENDING'\r\n"
			+ "  AND evm.encroachment_type = ?\r\n"
			+ "  AND evm.contact_number = ?\r\n"
			+ "  AND evm.number_of_violation = ?\r\n"
			+ "  AND (\r\n"
			+ "    evm.encroachment_type,\r\n"
			+ "    evm.contact_number,\r\n"
			+ "    evm.violator_name,\r\n"
			+ "    evm.number_of_violation,\r\n"
			+ "    TRIM(LOWER(evd.item_name))\r\n"
			+ ") IN (\r\n"
			+ "    SELECT \r\n"
			+ "        evm2.encroachment_type, \r\n"
			+ "        evm2.contact_number, \r\n"
			+ "        evm2.violator_name, \r\n"
			+ "        evm2.number_of_violation, \r\n"
			+ "        TRIM(LOWER(evd2.item_name)) AS itemname\r\n"
			+ "    FROM egec_challan_master ecm2\r\n"
			+ "    INNER JOIN egec_violation_master evm2 ON evm2.violation_uuid = ecm2.violation_uuid\r\n"
			+ "    INNER JOIN egec_violation_detail evd2 ON evd2.violation_uuid = evm2.violation_uuid\r\n"
			+ "    INNER JOIN egec_payment ep2 ON ep2.challan_uuid = ecm2.challan_uuid\r\n"
			+ "    WHERE ep2.payment_status = 'PENDING'\r\n"
			+ "      AND ecm2.tenant_id = 'ch.chandigarh'\r\n"
			+ "    GROUP BY \r\n"
			+ "        evm2.encroachment_type, \r\n"
			+ "        evm2.contact_number, \r\n"
			+ "        evm2.violator_name, \r\n"
			+ "        evm2.number_of_violation, \r\n"
			+ "        TRIM(LOWER(evd2.item_name))\r\n"
			+ "    HAVING COUNT(*) > 1\r\n"
			+ ")\r\n"
			+ "GROUP BY \r\n"
			+ "    ecm.challan_id, \r\n"
