...
public class ViolationService {
	 ...
	 * 
	 * @param RequestInfoWrapper containing objects of violation, violation details,
	 *                           documents
	 ...
	 */
	public ResponseEntity<ResponseInfoWrapper> generateChallan(RequestInfoWrapper requestInfoWrapper,
			...
			if (responseValidate.equals("")) {
				...
				if (id.getIdResponses() != null && id.getIdResponses().get(0) != null) {
					...
					throw new CustomException(HttpStatus.INTERNAL_SERVER_ERROR.toString(),
							EcConstants.FAILED_IDGEN_CHALLANID);
				}
				...
				violationMaster.getPaymentDetails()
						.setCreatedTime(requestInfoWrapper.getAuditDetails().getCreatedTime());
				violationMaster.getPaymentDetails()
				...
				if (response != null && response.getStatus().equalsIgnoreCase(EcConstants.STATUS_SUCCESSFULL)
					...
					// email notification
					// producer.push(config.getEmailNotificationTopic(),
					// violationMaster.getNotificationTemplate());
					// sms notification
	...
	public ResponseEntity<ResponseInfoWrapper> getChallan(RequestInfoWrapper requestInfoWrapper) {
		...
		try {
			...
			if (responseValidate.equals("")) {
				...
				return new ResponseEntity<>(ResponseInfoWrapper.builder()
						.responseInfo(ResponseInfo.builder().status(EcConstants.STATUS_SUCCESS).build())
						.responseBody(violationPage).build(), HttpStatus.OK);
			} else {
	...
	public ResponseEntity<ResponseInfoWrapper> addPayment(RequestInfoWrapper requestInfoWrapper) {
		...
		try {
			...
			if (responseValidate.equals("")) {
				...
				return new ResponseEntity<>(ResponseInfoWrapper.builder()
						.responseInfo(ResponseInfo.builder().status(EcConstants.STATUS_SUCCESS).build())
						.responseBody(ecPayment).build(), HttpStatus.OK);
			} else {
	...
	public ResponseEntity<ResponseInfoWrapper> updatePayment(RequestInfoWrapper requestInfoWrapper) {
		log.info("Violation Service - Add Payment");
		try {
			EcPaymentData ecPayment = objectMapper.convertValue(requestInfoWrapper.getRequestBody(),
					EcPaymentData.class);
			String responseValidate = "";
	...
	public ResponseEntity<ResponseInfoWrapper> sendNotification(RequestInfoWrapper requestInfoWrapper) {
		...
		try {
			NotificationTemplate email = objectMapper.convertValue(requestInfoWrapper.getRequestBody(),
			...
			if (responseValidate.equals("")) {
				...
				return new ResponseEntity<>(ResponseInfoWrapper.builder()
						.responseInfo(ResponseInfo.builder().status(EcConstants.STATUS_SUCCESS).build())
						.responseBody(email).build(), HttpStatus.OK);
			} else {
	...
	public ResponseEntity<ResponseInfoWrapper> getSearchChallanCount(RequestInfoWrapper requestInfoWrapper) {
		...
		try {
			...
			ViolationCount violation = objectMapper.convertValue(requestInfoWrapper.getRequestBody(),
					ViolationCount.class);
	...
	public ResponseEntity<ResponseInfoWrapper> sendMessage(RequestInfoWrapper requestInfoWrapper,
			String requestHeader) {
		log.info("Violation Service - Generate Challan");
		Violation violationMaster = objectMapper.convertValue(requestInfoWrapper.getRequestBody(), Violation.class);
		String strOutput = violationMaster.getNotificationTemplate().getBody()
