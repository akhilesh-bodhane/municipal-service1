...
public class WsQueryBuilder {
	...
	private static final String WATER_SEARCH_QUERY = "SELECT "
			...
			+ "  wc.connectionType, wc.waterSource,  wc.meterRentCode,   wc.sanctionedCapacity,"
			+ " wc.meterInstallationDate,  wc.noOfTaps, wc.proposedPipeSize, wc.proposedTaps, wc.connection_id as connection_Id, wc.connectionExecutionDate, "
			+ " wc.appCreatedDate,wc.proposed_meterid, wc.proposed_meterinstallationdate,wc.proposed_initialmeterreading,wc.proposed_lastmeterreading,wc.proposed_metercount,wc.proposed_meterrentcode,wc.proposed_mfrcode,wc.proposed_meterdigits,  wc.proposed_sanctionedcapacity,wc.proposed_meterunit,"
			+ " py.paymentmode,py.lastmodifiedtime as paymentdate ,"
			...
			+ " conn.action,  conn.applicationType, conn.dateEffectiveFrom,"
			+ " conn.div, conn.subdiv, conn.ledger_no,conn.ledgergroup, conn.createdBy as ws_createdBy, conn.lastModifiedBy as ws_lastModifiedBy,"
			+ " conn.createdTime as ws_createdTime, conn.lastModifiedTime as ws_lastModifiedTime, "
//			+ " conn.roadtype,application.application_code, conn.adhocpenaltyreason,"
//			+ "property.plotareatt as ploatAreaTT, wc.connectionCategory, conn.adhocpenaltycomment, conn.adhocrebatereason,"
//			+ " wc.estimationLetterDate, conn.adhocrebate,wc.detailsprovidedby, wc.estimationfileStoreId , wc.sanctionfileStoreId ,"
//			+ "conn.adhocrebatecomment, conn.adhocpenalty, wc.lastmeterreading, wc.initialmeterreading,wc.meterId, wc.meterUnit, "
//			+ "application.waterchargestt as waterChargesTT,"
//			+ "wc.pipeSize, conn.connectionusagestype, conn.cccode, wc.meterCount,  wc.mfrCode, wc.meterDigits,  conn.aadharNo, conn.ferruleSize,"
//			+ ""
			+ "conn.proposedUsage_category, conn.waterApplicationType, conn.securityCharge,  conn.inworkflow, conn.billGroup, conn.contract_value, "
			...
			+ " application.id as application_id, application.applicationno as app_applicationno, application.activitytype as app_activitytype, application.applicationstatus as app_applicationstatus, application.action as app_action, "
			+ "application.comments as app_comments, application.is_ferrule_applicable as app_ferrule, application.security_charges as app_securitycharge, application.total_amount_paid, application.additionalcharges, application.constructioncharges, application.outstandingcharges,  "
			+ "application.ismeterstolen, "
...
//		}
	...
	public String getSearchQueryStringTotalCollectionCountNIUA( String string ,
		...
		preparedStatement.add(searchTotalCollectionCriteria.getToDate());
			...
			else if ("todaysTotalApplications".equalsIgnoreCase(string)  && groupByName.equalsIgnoreCase("todaysTotalApplications")) {
			...
			else if ("todaysClosedApplications".equalsIgnoreCase(string)  && groupByName.equalsIgnoreCase("todaysClosedApplications")) {
