...
@Component
public class AlfMemberRowMapper implements ResultSetExtractor<List<SmidAlfMemberApplication>> {

	@Autowired
	private ObjectMapper mapper;

	@Override
	public List<SmidAlfMemberApplication> extractData(ResultSet rs) throws SQLException, DataAccessException {
		Map<String, SmidAlfMemberApplication> memberMap = new HashMap<>();
		List<SmidAlfMemberApplication> memberList = new ArrayList<>();
		...
		try {
			while (rs.next()) {
				...
				if (!memberMap.containsKey(id)) {
					...
					SmidAlfMemberApplication smidapp = SmidAlfMemberApplication.builder().auditDetails(audit)
							...
							.applicationStatus(
									SmidAlfMemberApplication.StatusEnum.fromValue(rs.getString("application_status")))
							...
							.remark(rs.getString("remark")).alfUuid(rs.getString("alf_uuid"))
